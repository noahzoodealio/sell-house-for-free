...
Legacy migration: when --legacy-dir is provided, reads the old per-module config
file from {legacy-dir}/{module-code}/config.yaml. Matching values serve as
fallback defaults (answers override them). After a successful merge, that legacy
config.yaml is deleted. Only the current module's own legacy staging is touched —
{legacy-dir}/core/ and every other module directory are left alone.
...
def load_legacy_values(
    ...
    Reads {legacy_dir}/{module_code}/config.yaml. Only returns values whose keys
    match the current schema (core keys or module.yaml variable definitions).
    Other modules' directories — including the BMad core module's own
    {legacy_dir}/core/ — are never touched.
...
def cleanup_legacy_configs(
    ...
    """Delete legacy config.yaml for this module only.

    Never touches {legacy_dir}/core/ — that belongs to the BMad core module.
